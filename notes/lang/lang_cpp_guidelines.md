...
### SF.3: 对在多个源文件中使用的任何声明，都应使用 `.h` 文件
...
### SF.5: `.cpp` 文件必须包含定义了它的接口的一个或多个 `.h` 文件
