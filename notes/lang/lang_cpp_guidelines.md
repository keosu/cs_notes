...
当提出您的意见时，请关注[导言部分，其中概述了我们的目标和所采用的一般方法。
贡献者的列表请参见[这里。
...
其中的 "tag" 是包含强制规则的条目的锚定名字（例如，134 的锚定名字为 "Rh-public"），
...
* 2: 用 ISO 标准 C++ 来编码
* 3: 表达你的设计意图
* 4: 理想情况下，程序应当是静态类型安全的
* 5: 编译期检查优先于运行时检查
* 6: 应当使无法在编译期进行的检查能够在运行时实施
* 7: 尽早识别运行时错误
* 8: 不要泄漏任何资源
* 9: 不要浪费时间或空间
* 10: 不可变数据优先于可变数据
* 11: 把杂乱的构造封装起来，而别让其散布到代码中
* 12: 适当采用支持工具
* 13: 适当采用支持程序库
...
* 4: 使接口严格和强类型化
* 5: 说明前条件（如果有）
* 6: 优先使用 `Expects()` 来表达前条件
* 7: 说明后条件
* 8: 优先使用 `Ensures()` 来表达后条件
* 9: 当接口是模板时，用概念来文档化其参数
* 10: 使用异常来表明无法实施所要求的任务
* 11: 决不以原始指针（`T*`）或引用（`T&`）来传递所有权
* 12: 把不能为空的指针声明为 `not_null`
* 13: 不要只用一个指针来传递数组
* 22: 避免全局对象之间进行复杂的初始化
* 23: 保持较少的函数参数数量
* 24: 若改变实参顺序可能改变其含义，则避免出现相邻的相同类型的形参
* 25: 优先以抽象类作为类层次的接口
* 26: 当想要跨编译器的 ABI 时，使用一个 C 风格的语言子集
* 27: 对于稳定的程序库 ABI，考虑使用 Pimpl 手法
* 30: 将有违规则的部分封装
...
使用 `unsigned` 并不是回避[确保非负数值问题的好方法。
...
* 一条对性能关键代码的好规则是，把检查从代码的[关键部分中移出去。
* 长期来看，更规整的代码会得到更好的优化。
* 在做出性能相关的声明前一定要小心地[进行测量。

**参见**: [I.5 有关报告前条件和后条件的违反。
...
**替代方案**: 用“智能指针”来[传递所有权，比如 `unique_ptr`（专有所有权）和 `shared_ptr`（共享所有权）。
这样做比返回对象自身来说并没有那么简炼，而且通常也不那么高效，
因此，仅当需要引用语义时再使用智能指针。

**替代方案**: 有时候因为 ABI 兼容性的要求或者缺少资源，是无法对老代码进行修改的。
这种情况下，请用[指导方针支持库的 `owner` 来标记拥有对象的指针：
...
但这样做时，应当使用 `std::string_view` 或 [GSL 中的 `string_span` 以避免范围错误。
...
### I.24: 若改变实参顺序可能改变其含义，则避免出现相邻的相同类型的形参
...
参见 [GOTW #100 有关这个手法相关的权衡和其他实现细节。
...
* 1: 把有意义的操作“打包”成为精心命名的函数
* 2: 一个函数应当实施单一一项逻辑操作
* 3: 保持函数短小简洁
* 4: 如果函数必须在编译期进行求值，就将其声明为 `constexpr`
* 5: 如果函数非常小，并且是时间敏感的，就将其声明为 `inline`
* 6: 如果函数不会抛出异常，就将其声明为 `noexcept`
* 7: 对于常规用法，应当接受 `T*` 或 `T&` 参数而不是智能指针
* 8: 优先采用纯函数
* 9: 未使用的形参应当没有名字

参数传递表达式的规则：

* 15: 优先采用简单的和传统的信息传递方式
* 16: 对于“输入（in）”参数，把复制操作廉价的类型按值进行传递，把其他类型按 `const` 引用进行传递
* 17: 对于“输入/输出（in-out）”参数，按非 `const` 引用进行传递
* 18: 对于“将被移动（will-move-from）”参数，按 `X&&` 进行传递并对参数 `std::move`
* 19: 对于“转发（forward）”参数，按 `TP&&` 进行传递并只对参数 `std::forward`
* 20: 对于“输出（out）”值，采用返回值优先于输出参数
* 21: 要返回多个“输出”值，优先返回结构体或元组（tuple）
* 60: 当“没有参数”是有效的选项时，采用 `T*` 优先于 `T&`

参数传递语义的规则：

* 22: 用 `T*` 或 `owner<T*>` 来代表单个对象
* 23: 用 `not_null<T>` 来表明“空值（null）”不是有效的值
* 24: 用 `span<T>` 或者 `span_p<T>` 来代表一个半开序列
* 25: 用 `zstring` 或者 `not_null<zstring>` 来代表 C 风格的字符串
* 26: 当需要指针时，用 `unique_ptr<T>` 来传递所有权
* 27: 用 `shared_ptr<T>` 来共享所有权

值返回语义的规则：

* 42: 返回 `T*` 来（仅仅）给出一个位置
* 43: 不要（直接或间接）返回指向局部对象的指针或引用
* 44: 当不想进行复制，且不需要“没有对象被返回”时，返回 `T&`
* 45: 不要返回 `T&&`
* 46: `int` 是 `main()` 的返回类型
* 47: 赋值运算符返回 `T&`
* 48: 不要用 `return std::move(local)`

其他函数规则：

* 50: 当函数不适用时（不能俘获局部变量，或者不能编写局部函数），就使用 Lambda
* 51: 如果需要作出选择，采用默认实参应当优先于进行重载
* 52: 对于局部使用的（也包括传递给算法的）lambda，优先采用按引用俘获
* 53: 对于非局部使用的（包括被返回的，在堆上存储的，或者传递给别的线程的）lambda，避免采用按引用俘获
* 54: 当俘获了 `this` 时，显式俘获所有的变量（不使用默认俘获）
* 55: 不要使用 `va_arg` 参数
 ### F.7: 对于常规用法，应当接受 `T*` 或 `T&` 参数而不是智能指针

- 理由

智能指针的传递会转移或者共享所有权，因此应当仅在有意要实现所有权语义时才能使用（参见 [R.30）。
...
* 有名字的非泛型 lambda（比如 `auto x = [ { /*...*/; };`），而其并未发生俘获并且出现于全局作用域，对它们给出警告。代之以编写常规的函数。
...
* 1: 把相关的数据组织到结构中（`struct` 或 `class`）
* 2: 当类具有不变式时使用 `class`；当数据成员可以独立进行变动时使用 `struct`
* 3: 用类来表示接口和实现之间的区别
* 4: 仅当函数直接访问类的内部表示时才让函数作为其成员
* 5: 把辅助函数放在其所支持的类相同的命名空间之中
* 7: 不要在同一个语句中同时定义类或枚举并声明该类型的变量
* 8: 当有任何非公开成员时使用 `class` 而不是 `struct`
* 9: 让成员的暴露最小化
...
* 20: 只要可能，请避免定义任何的默认操作
* 21: 如果定义或者 `=delete` 了任何复制、移动或析构函数，请定义或者 `=delete` 它们全部
* 22: 使默认操作之间保持一致

析构函数的规则：

* 30: 如果一个类需要在对象销毁时执行明确的操作，请为其定义析构函数
* 31: 类所获取的所有资源，必须都在类的析构函数中进行释放
* 32: 如果类中带有原始指针（`T*`）或者引用（`T&`），请考虑它是否是所有者
* 33: 如果类中带有所有权的指针成员，请定义析构函数
* 35: 基类的析构函数应当要么是 public 和 virtual，要么是 protected 且非 virtual
* 36: 析构函数不能失败
* 37: 使析构函数 `noexcept`

构造函数的规则：

* 40: 如果类具有不变式，请为其定义构造函数
* 41: 构造函数应当创建经过完整初始化的对象
* 42: 当构造函数无法构造有效对象时，应当抛出异常
* 43: 保证可复制（值类型）类带有默认构造函数
* 44: 尽量让默认构造函数简单且不抛出异常
* 45: 不要定义仅对数据成员进行初始化的默认构造函数；应当使用成员初始化式
* 46: 默认情况下，把单参数的构造函数声明为 `explicit`
* 47: 按成员声明的顺序对成员变量进行定义和初始化
* 48: 对于常量初始化式来说，优先采用类中的初始化式而不是构造函数中的成员初始化式
* 49: 优先进行初始化而不是在构造函数中赋值
* 50: 当初始化过程中需要体现“虚函数行为”时，请使用工厂函数
* 51: 用委派构造函数来表示类中所有构造函数的共同行为
* 52: 使用继承构造函数来把构造函数引入到无须进行其他的明确初始化操作的派生类之中

复制和移动的规则：

* 60: 使复制赋值非 `virtual`，接受 `const&` 的参数，并返回非 `const` 的引用
* 61: 复制操作应当进行复制
* 62: 使复制赋值可以安全进行自赋值
* 63: 使移动赋值非 `virtual`，接受 `&&` 的参数，并返回非 `const` 的引用
* 64: 移动操作应当进行移动，并使原对象处于有效状态
* 65: 使移动赋值可以安全进行自赋值
* 66: 使移动操作 `noexcept`
* 67: 多态类应当抑制复制操作
...
* 80: 当需要明确使用缺省语义时，使用 `=default`
* 81: 当需要关闭缺省行为（且不需要替代的行为）时，使用 `=delete`
* 82: 不要在构造函数和析构函数中调用虚函数
* 83: 考虑为值类型提供 `noexcept` 的 `swap` 函数
* 84: `swap` 不应当失败
* 85: 使 `swap` 函数 `noexcept`
* 86: 使 `==` 对操作数的类型对称，并使之 `noexcept`
* 87: 请当心基类的 `==`
* 89: 使 `hash` 函数 `noexcept`
* 90: 依靠构造函数和赋值运算符，不要依靠 `memset` 和 `memcpy`
...
对于大多数类来说，答案是“不需要”，要么是因为类中并没有保持任何资源，要么是因为销毁过程已经被[零之准则处理掉了；
就是说，它的成员在销毁之中可以自己照顾自己。
当答案为“需要”时，类的大部分设计应当遵循下列规则（参见[五之准则）。
...
如果构造函数（为创建有效的对象）获取了某个资源，则这个资源应当[由析构函数释放。
...
对于同时是可复制的类型来说，默认构造函数通常会简化定义一个适当的[移动遗留状态的任务。

- 注解

[值类型，是可以复制（且通常也可以比较）的类型。
这与 [EoP 中的正规（Regular）类型紧密相关。
...
[自赋值 return *this;` 测试的“每一百万次有一次”的论点，在自移动的情况中更加适当。
...
当需要创建多态对象的深拷贝副本时，应当使用 `clone()` 函数：参见 130。
...
构造某个类型的实例的标准 C++ 机制是调用其构造函数。如指导方针 41 所述：构造函数应当创建一个已完全初始化的对象。不应当需要进行如用 `memcpy` 来进行的额外初始化。
...
* 101: 为容器提供值语义
* 102: 为容器提供移动操作
* 103: 为容器提供一个初始化式列表构造函数
* 104: 为容器提供一个将之置空的默认构造函数
* ???
* 109: 当资源包装类具有指针语义时，应提供 `*` 和 `->`
...
* 120: 类层次（仅）用于表达具有天然层次化结构的概念
* 121: 如果基类被用作接口的话，应使其成为纯抽象类
* 122: 当需要完全区分接口和实现时，应当用抽象类作为接口

类层次的设计规则概览：

* 126: 抽象类通常并不需要构造函数
* 127: 带有虚函数的类应当带有虚的或受保护的析构函数
* 128: 虚函数应当指明 `virtual`、`override`、`final` 三者之一
* 129: 当设计类层次时，应区分实现继承和接口继承
* 130: 多态类的深拷贝；优先采用虚函数 `clone` 来替代复制构造/赋值
* 131: 避免无价值的取值和设值函数
* 132: 请勿无理由地使函数 `virtual`
* 133: 避免 `protected` 数据
* 134: 确保所有非 `const` 数据成员有相同的访问级别
* 135: 用多继承来表达多个不同的接口
* 136: 用多继承来表达一些实现特性的合并
* 137: 用 `virtual` 基类以避免过于通用的基类
* 138: 用 `using` 来为派生类和其基类建立重载集合
* 139: 对类运用 `final` 应当保守
* 140: 不要在虚函数和其覆盖函数上提供不同的默认参数

对类层次中的对象进行访问的规则概览：

* 145: 通过指针和引用来访问多态对象
* 146: 当无法避免在类层次上进行导航时应使用 `dynamic_cast`
* 147: 当查找所需类的失败被当做一种错误时，应当对引用类型使用 `dynamic_cast`
* 148: 当查找所需类的失败被当做一种有效的可能情况时，应当对指针类型使用 `dynamic_cast`
* 149: 用 `unique_ptr` 或 `shared_ptr` 来避免忘记对以 `new` 所创建的对象进行 `delete` 的情况
* 150: 用 `make_unique()` 来构建由 `unique_ptr` 所拥有的对象
* 151: 用 `make_shared()` 来构建由 `shared_ptr` 所拥有的对象
* 152: 禁止把指向派生类对象的数组的指针赋值给指向基类的指针
* 153: 优先采用虚函数而不是强制转换
...
带有虚函数的类通常是通过指向基类的指针来使用的。一般来说，最后一个使用者必须在基类指针上执行 `delete`，这常常是通过基类智能指针来做到的，因而析构函数应当为 `public` 和 `virtual`。而不那么常见的情况是当并不打算支持通过基类指针来删除时，这时析构函数应当为 `protected` 和非 `virtual`；参见 35。
...
由于切片的问题，不鼓励多态类的复制操作，参见 67。如果确实需要复制语义的话，应当进行深复制：提供一个虚 `clone` 函数，它复制的是真正的最终派生类型，并返回指向新对象的具有所有权的指针，而且在派生类中它返回的也是派生类型（利用协变返回类型）。
...
通常来说，推荐使用智能指针来表示所有权（参见 [R.20中的 `owner<>` 工具类。
...
注意，我们可以为成员变量提供默认初始化式：49: 优先进行初始化而不是在构造函数中赋值.
...
应当[优先采用 `private` 数据并提供良好定义并强加的不变式。
...
`Interface` 是一个[接口层次的根，
而 `Utility` 则是一个[实现层次的根。
[一个稍微更现实的例子，有一些解释。
...
[优先使用虚函数而不是强制转换。
如果可行（无须运行时决议）并且相当便利的话，优先使用[静态多态而不是
继承层次的导航。
...
比如说，在合适的条件下，是可以以[快速常量时间来实施 `dynamic_cast` 的。
...
* 160: 定义运算符应当主要用于模仿传统用法
* 161: 对于对称的运算符应当采用非成员函数
* 162: 重载的操作之间应当大体上是等价的
* 163: 应当仅对大体上等价的操作进行重载
* 164: 避免隐式转换运算符
* 165: 为定制点采用 `using`
* 166: 一元 `&` 的重载只能作为某个智能指针或引用系统的一部分而提供
* 167: 应当为带有传统含义的操作提供运算符
* 168: 应当在操作数所在的命名空间中定义重载运算符
* 170: 当想要重载 lambda 时，应当使用泛型 lambda
...
* 180: 采用 `union` 用以节省内存
* 181: 避免 `naked union` : 是没有相关的指示其持有哪个成员（如果有）的指示器的联合体，类型错误的一种来源
* 182: 利用匿名 `union` 实现带标记联合体
* 183: 不要将 `union` 用于类型双关 
...
* 1: 优先采用枚举而不是宏
* 2: 采用枚举来表示相关的具名常量的集合
* 3: 优先采用 `enum class` 而不是“普通”`enum` (传统的 `enum` 太容易转换为 `int` )
* 4: 针对安全和简单的用法来为枚举定义操作
* 5: 请勿为枚举符采用 `ALL_CAPS` 命名方式
* 6: 避免使用无名枚举
* 7: 仅在必要时才为枚举指定其底层类型
* 8: 仅在必要时才指定枚举符的值 (如：底层类型可以节省空间)
...
* 资源管理规则概览：
  ...
  * 1: 利用资源句柄和 RAII（资源获取即初始化）来自动管理资源
  * 2: 接口中的原生指针（仅）代表个体对象
  * 3: 原生指针（`T*`）没有所有权
  * 4: 原生引用（`T&`）没有所有权
  * 5: 优先采用有作用域的对象，避免不必要的堆分配
  * 6: 避免非 `const` 的全局变量

* 分配和回收规则概览：

  * 10: 避免 `malloc()` 和 `free()`
  * 11: 避免显式调用 `new` 和 `delete`
  * 12: 显式资源分配的结果应当立即交给一个管理对象
  * 13: 单个表达式语句中至多进行一次显式资源分配
  * 14: 避免使用 `[]` 形参，优先使用 `span`
  * 15: 总是同时重载相匹配的分配、回收函数对

* 智能指针规则概览：

  * 20: 用 `unique_ptr` 或 `shared_ptr` 表示所有权
  * 21: 优先采用 `unique_ptr` 而不是 `shared_ptr`，除非需要共享所有权
  * 22: 使用 `make_shared()` 创建 `shared_ptr`
  * 23: 使用 `make_unique()` 创建 `unique_ptr`
  * 24: 使用 `std::weak_ptr` 来打断 `shared_ptr` 的循环引用
  * 30: 以智能指针为参数，仅用于明确表达生存期语义
  * 31: 非 `std` 的智能指针，应当遵循 `std` 的行为模式
  * 32: `unique_ptr<widget>` 参数用以表达函数假定获得 `widget` 的所有权
  * 33: `unique_ptr<widget>&` 参数用以表达函数对该 `widget` 重新置位
  * 34: `shared_ptr<widget>` 参数用以表达函数是所有者的一份子
  * 35: `shared_ptr<widget>&` 参数用以表达函数可能会对共享的指针重新置位
  * 36: `const shared_ptr<widget>&` 参数用以表达它可能将保留一个对对象的引用 ???
  * 37: 不要把来自某个智能指针别名的指针或引用传递出去
...
上面两段根据 [`sharedptrparam` 指导方针来说都是错误的：
...
* 1: 优先采用标准库而不是其他的库或者“手工自制代码”
* 2: 优先采用适当的抽象而不是直接使用语言功能特性

声明的规则：

* 5: 保持作用域尽量小
* 6: 在 for 语句的初始化式和条件中声明名字以限制其作用域
* 7: 保持常用的和局部的名字尽量简短，而让非常用的和非局部的名字较长
* 8: 避免使用看起来相似的名字
* 9: 避免 `ALL_CAPS` 式的名字
* 10: 每条声明中（仅）声明一个名字
* 11: 使用 `auto` 来避免类型名字的多余重复
* 12: 不要在嵌套作用域中重用名字
* 20: 坚持为对象进行初始化
* 21: 不要在确实需要使用变量（或常量）之前就引入它
* 22: 要等到获得了用以初始化变量的值之后才声明变量
* 23: 优先使用 `{}` 初始化式语法
* 24: 用 `unique_ptr<T>` 来保存指针
* 25: 应当将对象声明为 `const` 或 `constexpr`，除非后面需要修改其值
* 26: 不要用一个变量来达成两个不相关的目的
* 27: 使用 `std::array` 或 `stack_array` 作为栈上的数组
* 28: 为复杂的初始化（尤其是 `const` 变量）使用 lambda
* 30: 不要用宏来操纵程序文本
* 31: 不要用宏来作为常量或“函数”
* 32: 对所有的宏名采用 `ALL_CAPS` 命名方式
* 33: 如果必须使用宏的话，请为之提供唯一的名字
* 34: 不要定义（C 风格的）变参函数
...
* 45: 避免“魔法常量”，采用符号化常量
* 46: 避免窄化转换
* 47: 使用 `nullptr` 而不是 `0` 或 `NULL`
* 48: 避免强制转换
* 49: 当必须使用强制转换时，使用具名的强制转换
* 50: 不要强制掉 `const`
* 55: 避免发生对范围检查的需要
* 56: 仅在确实需要明确移动某个对象到别的作用域时才使用 `std::move()`
* 60: 避免在资源管理函数之外使用 `new` 和 `delete`
* 61: 用 `delete[]` 删除数组，用 `delete` 删除非数组对象
* 62: 不要在不同的数组之间进行指针比较
* 63: 不要产生切片
* 64: 使用 `T{e}` 写法来进行构造
* 65: 不要解引用无效指针
...
* 70: 面临选择时，优先采用 `switch` 语句而不是 `if` 语句
* 71: 面临选择时，优先采用范围式 `for` 语句而不是普通 `for` 语句
* 72: 当存在显然的循环变量时，优先采用 `for` 语句而不是 `while` 语句
* 73: 当没有显然的循环变量时，优先采用 `while` 语句而不是 `for` 语句
* 74: 优先在 `for` 语句的初始化部分中声明循环变量
* 75: 避免使用 `do` 语句
* 76: 避免 `goto`
* 77: 尽量减少循环中使用的 `break` 和 `continue`
* 78: 不要依靠 `switch` 语句中的隐含直落行为
* 79: `default`（仅）用于处理一般情况
* 84: 不要试图声明没有名字的局部变量
* 85: 让空语句显著可见
* 86: 避免在原生的 `for` 循环中修改循环控制变量
* 87: 请勿在条件上添加多余的 `==` 或 `!=`
...
* 104: 避免下溢出
* 105: 避免除零
* 106: 不要试图用 `unsigned` 来防止负数值
* 107: 不要对下标使用 `unsigned`，优先使用 `gsl::index`
...
一次移动通常会遗留一个空对象（64），这可能令人意外甚至很危险，因此我们试图避免从左值进行移动（它们可能随后会被访问到）。
...
而这点之后，应当假定对象已经被移走（参见 64），而直到首次向它设置某个新值之前，请勿再次读取它的状态。
...
通常来说，请不要无理由地使代码复杂化。(??)
绝不要在 const 对象上 `std::move()`，它只会暗中将其转变成一个副本（参见 [Meyers15。
...
* 1: 假定你的代码将作为多线程程序的一部分而运行
* 2: 避免数据竞争
* 3: 最小化可写数据的明确共享
* 4: 以任务而不是线程的角度思考
* 8: 不要为同步而使用 `volatile`
* 9: 只要可行，就使用工具对并发代码进行验证
...
* 20: 使用 RAII，绝不使用普通的 `lock()`/`unlock()`
* 21: 用 `std::lock()` 或 `std::scoped_lock` 来获得多个 `mutex`
* 22: 绝不在持有锁的时候调用未知的代码（比如回调）
* 23: 把联结的 `thread` 看作是有作用域的容器
* 24: 把 `thread` 看作是全局的容器
* 25: 优先采用 `gsl::joining_thread` 而不是 `std::thread`
* 26: 不要 `detach()` 线程
* 31: 少量数据在线程之间按值传递，而不是通过引用或指针传递
* 32: 用 `shared_ptr` 在无关的 `thread` 之间共享所有权
* 40: 最小化上下文切换
* 41: 最小化线程的创建和销毁
* 42: 不要无条件地 `wait`
* 43: 最小化临界区的时间耗费
* 44: 记得为 `lock_guard` 和 `unique_lock` 命名
* 50: `mutex` 要和其所护卫的数据一起定义。一旦可能就使用 `synchronized_value<T>`
* ??? 何时使用 spinlock
* ??? 何时使用 `try_lock()`
...
双检查锁定是很容易被搞乱的。如果确实需要编写自己的双检查锁定，而不顾规则 [CP.110: 不要为初始化编写你自己的双检查锁定，那么应当采用惯用的模式。

使用双检查锁定模式而不违反[CP.110: 不要为初始化编写你自己的双检查锁定的情形，出现于当某个非线程安全的动作既困难也罕见，并且存在某个快速且线程安全的测试可以用于保证该动作并不需要实施的情况，但反过来的情况则无法保证。
...
* 1: 在设计中尽早开发错误处理策略
* 2: 通过抛出异常来明示函数无法完成其所赋予的任务
* 3: 仅使用异常来进行错误处理
* 4: 围绕不变式来设计错误处理策略
* 5: 让构造函数建立不变式，若其无法做到则抛出异常
* 6: 使用 RAII 来避免泄漏
* 7: 明示前条件
* 8: 明示后条件

* 12: 当函数不可能或不能接受以 `throw` 来退出时，使用 `noexcept`
* 13: 不要在作为某个对象的直接所有者时抛出异常
* 14: 应当使用为目的所设计的自定义类型（而不是内建类型）作为异常
* 15: 按引用捕获类型层次中的异常
* 16: 析构函数，回收函数，以及 `swap` 决不能失败
* 17: 不要试图在每个函数中捕获每个异常
* 18: 最小化对 `try`/`catch` 的显式使用
* 19: 当没有合适的资源包装时，使用 `final_action` 对象来表达清理动作

* 25: 当不能抛出异常时，模拟 RAII 来进行资源管理
* 26: 当不能抛出异常时，考虑采取快速失败
* 27: 当不能抛出异常时，系统化地使用错误代码
* 28: 避免基于全局状态（比如 `errno`）的错误处理
...
重新抛出已捕获的异常应当使用 `throw;` 而非 `throw e;`。使用 `throw e;` 将会抛出 `e` 的一个新副本（并切片成静态类型 `std::exception`），而并非重新抛出原来的 `std::runtime_error` 类型的异常。（但请关注[请勿试图在每个函数中捕获所有的异常

- 强制实施

当异常类型属于某个类型层次时，对按值捕获异常进行标记（这可能需要全程序分析才能完美达成这点）。

### E.16: 析构函数，回收函数，以及 `swap` 决不能失败
...
相对于老式的 [`goto exit;` 技巧来说，使用 `finally` 是处理并非系统化的资源管理中的清理工作的
更加系统化并且相当简洁的方案。
...
* 1: 缺省情况下，对象应当是不可变的
* 2: 缺省情况下，成员函数应当为 `const`
* 3: 缺省情况下，应当传递指向 `const` 对象的指针或引用
* 4: 构造之后不再改变其值的对象应当以 `const` 来定义
* 5: 以 `constexpr` 来定义可以在编译期计算的值
...
“概念”是在一份 ISO 技术规范中定义的：[concepts。
而一组标准库概念的草案则可以在另一份 ISO TS 中找到：[ranges。
GCC 6.1 及其后版本支持概念。
...
* [T.46: 要求模板参数至少为 `Regular` 或者 `SemiRegular`
...
“标准”概念（即由 [GSL，以及希望近期 ISO 标准自身所提供的）概念，
...
* 1: 优先使用 C++ 而不是 C
* 2: 当一定要用 C 时，应使用 C 和 C++ 的公共子集，并将 C 代码以 C++ 来编译
* 3: 当一定要用 C 来作为接口时，应在使用这些接口的调用方代码中使用 C++
 
# SF: 源文件


* 1: 如果你的项目还未采用别的约定的话，应当为代码文件使用后缀 `.cpp`，而对接口文件使用后缀 `.h`
* 2: `.h` 文件不应当包含对象定义或非内联的函数定义
* 3: 对在多个源文件中使用的任何声明，都应使用 `.h` 文件
* 4: 在文件中的其他所有声明之前包含 `.h` 文件
* 5: `.cpp` 文件必须包含定义了它的接口的一个或多个 `.h` 文件
* 6: `using namespace` 指令，（仅）可以为迁移而使用，可以为基础程序库使用（比如 `std`），或者在局部作用域中使用
* 7: 请勿在头文件中的全局作用域使用 `using namespace` 指令
* 8: 为所有的 `.h` 文件使用 `#include` 防卫宏
* 9: 避免源文件的循环依赖
...
* 1: 尽可能使用程序库
* 2: 优先使用标准库而不是其他程序库
* 3: 请勿向命名空间 `std` 中添加非标准实体
* 4: 以类型安全的方式使用标准库 
 
## SL.con: 容器 

* 1: 优先采用 STL 的 `array` 或 `vector` 而不是 C 数组
* 2: 除非有理由使用别的容器，否则默认情况应优先采用 STL 的 `vector`
* 3: 避免边界错误
* 4: 请勿对非可平凡复制的实参使用 `memset` 或 `memcpy`
...
* 1: 使用 `std::string` 以拥有字符序列
* 2: 使用 `std::string_view` 或 `gsl::string_span` 以指代字符序列
* 3: 使用 `zstring` 或 `czstring` 以指代 C 风格、以零结尾的字符序列
* 4: 使用 `char*` 以指代单个字符
* 5: 使用 `std::byte` 以指代并不必须表示字符的字节值

* 10: 当需要实施相关于文化地域的操作时，使用 `std::string`
* 11: 当需要改动字符串时，使用 `gsl::string_span` 而不是 `std::string_view`
* 12: 为作为标准库的 `string` 类型的字符串字面量使用后缀 `s`
...
* 1: 仅在必要时才使用字符层面的输入
* 2: 当进行读取时，总要考虑非法输入
* 3: 优先使用 iostream 进行 I/O
* 10: 除非你使用了 `printf` 族函数，否则要调用 `ios_base::sync_with_stdio(false)`
* 50: 避免使用 `endl`
...
* 3: 请勿避免使用异常
* 4: 请勿坚持把每个类声明放在其自己的源文件中
* 5: 请勿采用两阶段初始化
* 6: 请勿把所有清理操作放在函数末尾并使用 `goto exit`
* 7: 请勿使所有数据成员 `protected` 
...
* Type.1: [避免强制转换：
<a name="Pro-type-reinterpretcast">a. </a>请勿使用 `reinterpret_cast`；此为[避免强制转换的严格的版本。  
<a name="Pro-type-arithmeticcast">b. </a>请勿在算术类型上使用 `static_cast`；此为[避免强制转换的严格的版本。  
<a name="Pro-type-identitycast">c. </a>当源指针类型和目标类型相同时，请勿进行指针强制转换；此为[避免强制转换的严格的版本。  
<a name="Pro-type-implicitpointercast">d. </a>当指针转换可以隐式转换时，请勿使用指针强制转换；此为[避免强制转换的严格的版本。  
...
要注意的是，这种类型安全性仅当我们同样具有[边界安全性时才是完整的。
...
引用都不是所有者（参见 [R.4`，等等。生存期安全性剖面配置的目标就是处理这些事情，但即便如此 `owner<T&>` 也没有意义且不建议使用。
...
它们的写法依照 ISO WG21 的 [Concepts TS。
下列概念中的大多数都定义在 [Ranges TS 中。

* `Range`
* `String`   // ???
* `Number`   // ???
* `Sortable`
* `Pointer`  // 带有 `*`，`->`，`==`，以及默认构造的类型（默认构造被假定为设值为唯一的“null”值）；参见[智能指针
* `Unique_pointer`  // 符合 `Pointer` 的类型，具有移动（而不是复制）操作，并符合生存期剖面配置中针对 `unique` 所有者类型的准则；参见[智能指针
* `Shared_pointer`   // 符合 `Pointer` 的类型，具有复制操作，并符合生存期剖面配置中针对 `shared` 所有者类型的准则；参见[智能指针
* `EqualityComparable`
* `Convertible`
* `Common`
* `Boolean`
...
是在 [Bjarne Stroustrup 在他为 CppCon 2015 的开场主旨演讲，“Writing Good C++14”。
...
- 6: 这些指导方针被 ISO C++ 标准委员会采纳了吗？它们是否代表委员会的一致意见？

不是这样。这些指导方针不在标准之内。它们是为标准服务的，而当前维护的指导方针是为了更有效地使用当前的标准 C++的。我们的目标是使其与委员会所设计的标准保持同步。

- 7: 既然这些指导方针并不是委员会所采纳的，它们为何在 `github.com/isocpp` 之下呢？

因为 `isocpp` 是标准 C++ 基金会；而标准委员会的仓库则处于 [github.com/*cplusplus* 所做的相同模式，它是有 Bjarne Stroustrup，Marshall Cline，和 Herb Sutter 所发起的工作，并以相同的方式贡献为了开放项目。

- 8: 会有 C++98 版本的指导方针吗？C++11 版本呢？
...
- 10: 这些指导方针的书写使用的是哪个版本的 Markdown？
...
- 59: `Expects` 和 `assert` 一样吗？

不一样。它是一种对于契约前条件语言支持的占位符。

- 60: `Ensures` 和 `assert` 一样吗？
...
* *使用虚基类语义：* 语言规则要求由最终派生类的构造函数来决定调用哪个基类构造函数；你可以利用这点。（参见[\[Taligent94\]。）
...
> 当在栈展开过程中所调用的析构函数因为异常而退出时，将调用 terminate (15.5.1)。因此析构函数通常应当捕获异常，并防止它们被传播出析构函数。 --[\[C++03\]
>
> C++ 标准库中所定义的任何析构函数（也包括用于实例化标准库模板的任何类型的析构函数）的操作都不会抛出异常。 --[\[C++03\]
...
一般来说，工具是无法知道类是否是资源句柄的。不过，如果类带有某种[默认操作的话, 它就得拥有全部，而如果类中有成员为资源句柄的话，它也应被当做是资源句柄。
...
* 
  \[SuttHysl04b]:     H. Sutter and J. Hyslop. [Collecting Shared Objects.
* 
